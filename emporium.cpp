...
string Emporium::get_servers_report() {
	string out = "\n";
	out += "		Mavs Ice Cream Emporium Parlour -- " + to_string(_id) + "		" + "\n";
	out += "		" + _location + "		" + "\n";
	out += "		" + _phone + "		    "  + "\n\n";
	out += "SERVERS REPORT\n\n";
	out += "Active Servers: \n\n";
...
string Emporium::get_pnl_report() {
	...
	out += "		" + _location + "		" + "\n";
	out += "		" + _phone + "		    "  + "\n\n";
	out += "Current PROFIT AND LOSS STATEMENT\n\n";
...
void Emporium::write(string filename){
	...
	if(!ofs) throw runtime_error("Sorry, can't open output file ! " + filename);
...
void Emporium:: populate_emporium(){
    Containr cont("Cup", "A waffle cone.", 0.1, 0.6, "picture.png", 3);
    Containr cont2("Regular Cone", "All time Favroite", 0.2, 0.8, "picture.png", 1);

    _containers.push_back(cont2);
    _containers.push_back(cont);

    Flavor flav("Chocolate", "Dark Chocolate", 1, 1.99, "f_chocolate.png");
    Flavor flav2("Strawberry", "Strawberry", 1.05, 2.09, "f_strawberry.png");
		Flavor flav3("Vanilla", "Vanilla", 1, 1.99, "f_vanilla.png");
    ...
    Topping top3("Cherry", "Bright and delightful", 0.01, 0.1, "t_cherry.png");

		_managers.push_back(Manager{"Sanam", 0, "786-786-9898"});

    _toppings.push_back(top);
    _toppings.push_back(top2);
    _toppings.push_back(top3);

    _servers.push_back(Server{"Daniel", 0, "444-666-9981",  9.5});
		_servers.push_back(Server{"Yussef", 2, "817-876-9701",  8.5});
    _servers.push_back(Server{"Yolanda", 1, "817-666-9999",  8.5});
		_servers.push_back(Server{"Rosie", 3, "464-876-9981",  9.0});

    _customers.push_back(Customer{"Mr. White", 0, "469-897-9297"});
    _customers.push_back(Customer{"Gregory.", 1, "665-213-2234"});
		_customers.push_back(Customer{"Mr. Adams", 2, "469-877-1297"});
