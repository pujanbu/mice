...
Emporium emp{1,"Kathmandu", "469-555-8978"};
...
Main_window::Main_window() {
	...
	// new customer menu
	Gtk::MenuItem *menuitem_add_customerr = Gtk::manage(new Gtk::MenuItem("_Add New Customer", true));
	menuitem_add_customerr->signal_activate().connect(sigc::mem_fun(*this, &Main_window::on_new_customer));
	manager_menu->append(*menuitem_add_customerr);
	...
	// new serving menu
	Gtk::MenuItem *menuitem_stock = Gtk::manage(new Gtk::MenuItem("_Auto Stock", true));
	menuitem_stock->signal_activate().connect(sigc::mem_fun(*this, &Main_window::on_stock));
	server_menu->append(*menuitem_stock);
	...
	new_emporium_button->signal_clicked().connect(sigc::mem_fun(*this, &Main_window::on_new_mgmt_click));
	...
	create_order_button->signal_clicked().connect(sigc::mem_fun(*this, &Main_window::on_new_order));
	...
	//create_item_button->signal_clicked().connect(sigc::mem_fun(*this, &Main_window::on_create_item_click));
...
void Main_window::on_about_click() {   //shows company description
	...
	std::vector< Glib::ustring > authors = {"George Rice , Pujan Budhathoki"};
	dialog.set_authors(authors);
	std::vector< Glib::ustring > artists = {"Pictures from www.webstaurantstore.com/joy-40-cake-ice-cream-cone-case/\n711CAKECON40.html?utm_source=Google&utm_medium=cpc&utm_campaign=GoogleShopping&gclid=EAIaIQobChMIz\nrO40b3n2gIVDgFpCh002ACcEAQYBSABEgLab_D_BwE/ \n"};
